"""実験 0-P (Predictive モデルの理論とシミュレーションの整合性検証).

実験 0 (scripts/experiment_0_validation.py) と同じ実験計画 (c=20, K=200,
中バースト水準 delta=0.6, sigma=0.1, rho in [0.3, 0.95]) を用い,
MMPP/M/c/SET-BATCH/Delayoff/Predictive モデル (mmpp_predictive) の理論解析
(LU 分解) と DES (mmpp_predictive_sim, 独立レプリケーション法) の 6 指標
(P_block, E[N], E[W], lambda_eff, rho_server, Cost) を比較する.

ベースモデルの実験 0 とは異なり, n_target > 0, gamma > 1 の代表値を固定して
Predictive 拡張の 2 機構 (事前セットアップ, Delayoff 加速) が実際に働く
設定で検証する.

使用例:
    python scripts/experiment_0P_validation.py --n-target 10 --gamma 5.0 \
        --n-replications 20 --warmup-events 100000 --measure-events 1000000
    python scripts/experiment_0P_validation.py --quick  # 開発中の軽量実行
"""
import argparse
import os
...
from mmpp_predictive_sim import SimMetrics, run_replications
try:
...
def build_params(rho: float, n_target: int, gamma: float) -> PredictiveModelParameters:
    """rho に対応する PredictiveModelParameters を構築する (中バースト水準, BASELINE 固定)."""
    C0, C1 = build_mmpp(rho)
    return PredictiveModelParameters(C0=C0, C1=C1, n_target=n_target, gamma=gamma, **BASELINE)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--n-target", type=int, default=10, help="事前セットアップの目標稼働サーバー数")
    parser.add_argument("--gamma", type=float, default=5.0, help="Delayoff 加速係数 (F=0 で beta -> gamma*beta)")
    parser.add_argument("--n-replications", type=int, default=20, help="独立レプリケーション数")
    parser.add_argument("--warmup-events", type=int, default=100_000)
    parser.add_argument("--measure-events", type=int, default=1_000_000)
    parser.add_argument("--n-points", type=int, default=10, help="rho スイープ点数")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--out-dir", default="figures")
    parser.add_argument(
        "--quick", action="store_true",
        help="短時間で検証する場合のオプション (n-replications=3, warmup=10000, measure=100000, n-points=3)",
    )
    args = parser.parse_args()
    if args.quick:
        args.n_replications = 3
        args.warmup_events = 10_000
        args.measure_events = 100_000
        args.n_points = 3
    return args
...
def main() -> None:
    ...
    proactive_stats: List[Tuple[float, int, float]] = []  # (rho, count, mean_delta)

    for rho in rho_values:
        params = build_params(float(rho), args.n_target, args.gamma)
        theory = run_theory(params)
        replications, sim_metrics = run_simulation(
            params, args.seed, args.warmup_events,
            args.measure_events, args.n_replications,
        )
        ...
        # Predictive 固有の診断統計 (事前セットアップの発動回数・平均台数)
        # run_replications は SimStats のみ返し, 発動回数を記録しないため
        # (SimStats は mmpp_sim.stats を再利用しており拡張していない),
        # 代表的な発動頻度の目安として参考実行を別途 1 回行う.
        from mmpp_predictive_sim.simulator import PredictiveSimulator
        probe = PredictiveSimulator(params, seed=args.seed + 9999)
        probe.run(warmup_events=min(args.warmup_events, 5_000),
                  measurement_events=min(args.measure_events, 20_000))
        if probe.proactive_setup_count > 0:
            mean_delta = probe.proactive_setup_delta_sum / probe.proactive_setup_count
        else:
            mean_delta = 0.0
        proactive_stats.append((float(rho), probe.proactive_setup_count, mean_delta))
        print(f"  [事前セットアップ診断 (参考実行)] 発動回数={probe.proactive_setup_count}, "
              f"平均delta_eff={mean_delta:.3g}")
    ...
    print("事前セットアップ発動診断 (rho, count, mean_delta_eff):")
    for rho, count, mean_delta in proactive_stats:
        print(f"  rho={rho:.4g}: count={count}, mean_delta_eff={mean_delta:.3g}")
...
def _plot(
    ...
    fig.suptitle(
        f"Experiment 0-P: Theory vs Simulation (Predictive, medium burst, "
        f"delta={DELTA}, sigma={SIGMA}, n_target={args.n_target}, gamma={args.gamma}, {cv_label})"
    )
    fig.tight_layout(rect=(0, 0, 1, 0.95))

    os.makedirs(args.out_dir, exist_ok=True)
    out_path = os.path.join(args.out_dir, "experiment_0P_validation.png")
    pdf_path = os.path.join(args.out_dir, "experiment_0P_validation.pdf")
